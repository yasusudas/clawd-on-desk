...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      sidebarTelegramApproval: "Telegram Approval",
      telegramApprovalTitle: "Telegram Remote Approval",
      telegramApprovalSubtitle: "Send local permission prompts to your Telegram bot. Tokens stay outside prefs and the desktop bubble remains the local fallback.",
      telegramApprovalSectionRuntime: "Runtime",
      telegramApprovalSectionConfig: "Configuration",
      ...
      telegramApprovalBotTokenDesc: "Stored in the user-data env file, not in clawd-prefs.json. Existing tokens are never shown here.",
      ...
      telegramApprovalTokenStored: "Token configured",
      telegramApprovalTokenMissing: "Token missing",
      telegramApprovalAllowedUser: "Allowed Telegram user id",
      telegramApprovalAllowedUserHint: "Only this numeric Telegram user id may press Allow or Deny.",
      telegramApprovalTargetSession: "Target session key",
      telegramApprovalTargetSessionHint: "Use telegram:<chat_id>. Numeric chat id shorthand is accepted.",
      telegramApprovalSaveConfig: "Save config",
      ...
      telegramApprovalTestUnavailable: "Enable Telegram approval and complete config/token before sending a test.",
      telegramApprovalStatus_stopped: "Stopped",
      telegramApprovalStatus_starting: "Starting…",
      telegramApprovalStatus_running: "Running",
      telegramApprovalStatus_failed: "Failed",
      "telegramApprovalReason_invalid-config": "Config incomplete",
      "telegramApprovalReason_missing-token": "Token missing",
    ...
    zh: {
      ...
      sidebarTelegramApproval: "Telegram 审批",
      telegramApprovalTitle: "Telegram 远程审批",
      telegramApprovalSubtitle: "把本地权限请求同步发送到你的 Telegram bot。Token 不写入 prefs，本地权限气泡仍然保留作为兜底。",
      telegramApprovalSectionRuntime: "运行状态",
      telegramApprovalSectionConfig: "配置",
      telegramApprovalToggle: "启用 Telegram 审批",
      telegramApprovalToggleDesc: "配置完成后，可操作的本地权限气泡会同时发送一张 Telegram 审批卡片。",
      telegramApprovalBotToken: "Bot token",
      telegramApprovalBotTokenDesc: "保存到 userData env 文件，不写入 clawd-prefs.json。已保存的 token 不会在这里回显。",
      ...
      telegramApprovalTokenStored: "Token 已配置",
      telegramApprovalTokenMissing: "Token 缺失",
      telegramApprovalAllowedUser: "允许审批的 Telegram user id",
      telegramApprovalAllowedUserHint: "只有这个数字 user id 可以点击 Allow / Deny。",
      telegramApprovalTargetSession: "目标 session key",
      telegramApprovalTargetSessionHint: "使用 telegram:<chat_id>。也可以直接填数字 chat id。",
      telegramApprovalSaveConfig: "保存配置",
      ...
      telegramApprovalTestUnavailable: "请先启用 Telegram 审批，并完成配置和 token，再发送测试。",
      telegramApprovalStatus_stopped: "已停止",
      telegramApprovalStatus_starting: "启动中…",
      telegramApprovalStatus_running: "运行中",
      telegramApprovalStatus_failed: "失败",
      "telegramApprovalReason_invalid-config": "配置不完整",
      "telegramApprovalReason_missing-token": "缺少 token",
    ...
    "zh-TW": {
      ...
      sidebarTelegramApproval: "Telegram 審批",
      telegramApprovalTitle: "Telegram 遠端審批",
      telegramApprovalSubtitle: "將本機權限請求同步送到你的 Telegram bot。Token 不會寫入 prefs，本機權限泡泡仍保留為備援。",
      telegramApprovalSectionRuntime: "執行狀態",
      telegramApprovalSectionConfig: "設定",
      telegramApprovalToggle: "啟用 Telegram 審批",
      telegramApprovalToggleDesc: "設定完成後，可操作的本機權限泡泡也會送出 Telegram 審批卡片。",
      telegramApprovalBotToken: "Bot token",
      telegramApprovalBotTokenDesc: "儲存在 userData env 檔，不寫入 clawd-prefs.json。已儲存的 token 不會顯示。",
      ...
      telegramApprovalTokenStored: "Token 已設定",
      telegramApprovalTokenMissing: "Token 缺失",
      telegramApprovalAllowedUser: "允許審批的 Telegram user id",
      telegramApprovalAllowedUserHint: "只有這個數字 user id 可以點 Allow / Deny。",
      telegramApprovalTargetSession: "目標 session key",
      telegramApprovalTargetSessionHint: "使用 telegram:<chat_id>。也可直接填數字 chat id。",
      telegramApprovalSaveConfig: "儲存設定",
      ...
      telegramApprovalTestUnavailable: "請先啟用 Telegram 審批，並完成設定和 token，再送出測試。",
      telegramApprovalStatus_stopped: "已停止",
      telegramApprovalStatus_starting: "啟動中…",
      telegramApprovalStatus_running: "執行中",
      telegramApprovalStatus_failed: "失敗",
      "telegramApprovalReason_invalid-config": "設定不完整",
      "telegramApprovalReason_missing-token": "缺少 token",
    ...
    ko: {
      ...
      sidebarTelegramApproval: "Telegram 승인",
      telegramApprovalTitle: "Telegram 원격 승인",
      telegramApprovalSubtitle: "로컬 권한 요청을 Telegram bot으로 보냅니다. Token은 prefs에 저장하지 않으며 데스크톱 버블은 로컬 fallback으로 유지됩니다.",
      telegramApprovalSectionRuntime: "런타임",
      telegramApprovalSectionConfig: "설정",
      telegramApprovalToggle: "Telegram 승인 활성화",
      telegramApprovalToggleDesc: "설정되면 처리 가능한 로컬 권한 버블이 Telegram 승인 카드도 보냅니다.",
      telegramApprovalBotToken: "Bot token",
      telegramApprovalBotTokenDesc: "userData env 파일에 저장되며 clawd-prefs.json에는 저장되지 않습니다. 저장된 token은 표시하지 않습니다.",
      ...
      telegramApprovalTokenStored: "Token 설정됨",
      telegramApprovalTokenMissing: "Token 없음",
      telegramApprovalAllowedUser: "허용된 Telegram user id",
      telegramApprovalAllowedUserHint: "이 숫자 user id만 Allow / Deny를 누를 수 있습니다.",
      telegramApprovalTargetSession: "대상 session key",
      telegramApprovalTargetSessionHint: "telegram:<chat_id>를 사용하세요. 숫자 chat id만 입력해도 됩니다.",
      telegramApprovalSaveConfig: "설정 저장",
      ...
      telegramApprovalTestUnavailable: "테스트를 보내기 전에 Telegram 승인을 켜고 설정/token을 완료하세요.",
      telegramApprovalStatus_stopped: "정지됨",
      telegramApprovalStatus_starting: "시작 중…",
      telegramApprovalStatus_running: "실행 중",
      telegramApprovalStatus_failed: "실패",
      "telegramApprovalReason_invalid-config": "설정 미완성",
      "telegramApprovalReason_missing-token": "Token 없음",
    ...
    ja: {
      ...
      sidebarTelegramApproval: "Telegram 承認",
      telegramApprovalTitle: "Telegram リモート承認",
      telegramApprovalSubtitle: "ローカルの権限リクエストを Telegram bot に送ります。Token は prefs に保存せず、デスクトップの吹き出しはローカル fallback として残ります。",
      telegramApprovalSectionRuntime: "ランタイム",
      telegramApprovalSectionConfig: "設定",
      telegramApprovalToggle: "Telegram 承認を有効化",
      telegramApprovalToggleDesc: "設定済みの場合、操作可能なローカル権限吹き出しから Telegram 承認カードも送信します。",
      telegramApprovalBotToken: "Bot token",
      telegramApprovalBotTokenDesc: "userData env ファイルに保存し、clawd-prefs.json には保存しません。保存済み token は表示しません。",
      ...
      telegramApprovalTokenStored: "Token 設定済み",
      telegramApprovalTokenMissing: "Token 未設定",
      telegramApprovalAllowedUser: "許可する Telegram user id",
      telegramApprovalAllowedUserHint: "この数値 user id だけが Allow / Deny を押せます。",
      telegramApprovalTargetSession: "対象 session key",
      telegramApprovalTargetSessionHint: "telegram:<chat_id> を使います。数値 chat id だけでも入力できます。",
      telegramApprovalSaveConfig: "設定保存",
      ...
      telegramApprovalTestUnavailable: "テスト送信の前に Telegram 承認を有効化し、設定と token を完了してください。",
      telegramApprovalStatus_stopped: "停止",
      telegramApprovalStatus_starting: "起動中…",
      telegramApprovalStatus_running: "実行中",
      telegramApprovalStatus_failed: "失敗",
      "telegramApprovalReason_invalid-config": "設定が不完全",
      "telegramApprovalReason_missing-token": "Token 未設定",
