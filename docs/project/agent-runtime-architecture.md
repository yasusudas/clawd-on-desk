...
- `agents/pi.js` — Pi extension 事件映射 + 能力（state-only，无 permission bubble）
...
- `server.js` 启动后异步同步 Claude / Codex / Gemini / Cursor / CodeBuddy / Kiro / Kimi hooks、opencode plugin 和 Pi extension
...
- Pi permission bubble 是 opt-in：`prefs` 默认和 v1->v2 migration 都把 `agents.pi.permissionsEnabled` 置为 `false`
