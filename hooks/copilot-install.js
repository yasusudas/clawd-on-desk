...
function registerCopilotHooks(options = {}) {
  ...
  let settings = {};
  try {
    // Strip the UTF-8 BOM (U+FEFF) if present before JSON.parse — PowerShell's
    // `Set-Content -Encoding utf8` and some Notepad saves prepend it, and
    // Node's JSON.parse refuses to parse a leading BOM. Without this guard,
    // any user who hand-edits hooks.json through those tools silently breaks
    // every later Clawd sync attempt (the throw here is caught by the
    // wrapping syncCopilotHooks try/catch, so the user just sees "Clawd
    // hooks not registered" with no fixable error path).
    let raw = fs.readFileSync(hooksPath, "utf-8");
    if (raw.charCodeAt(0) === 0xFEFF) raw = raw.slice(1);
